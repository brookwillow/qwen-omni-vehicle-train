...
| `data/tools.json`                                  | 39 个车载工具定义（新版`inputSchema` 格式，已包含 `IdentityControl`，已移除 `ParkingControl`）                                                                                                                                                                                                                                                                                                                                                              |
...
| `data/rl/wrong_tool_preferences.jsonl`             | 第二轮 DPO 偏好数据（44 条）：GT 工具和预测工具不同，且 chosen/rejected 均通过 schema 校验，用于修正工具混淆                                                                                                                                                                                                                                                                                                                                               |
| `data/rl/false_reject_clarify_preferences.jsonl`   | 第二轮 DPO 偏好数据（11 条）：有效工具请求被输出为 `Reject` 或澄清话术，`chosen=正确工具`、`rejected=Reject/Clarify`                                                                                                                                                                                                                                                                                                                                       |
...
| 类型                                   | 数量 | 说明                                                                                                                                                                                                                                                                                            |
| -------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| By-tool                                | 7265 | 每个工具文件内混合：Action JSON、NoiseDoNotAct、独立`JSON tool call -> tool-role JSON result -> TTS text` 回复样本；`NoiseDoNotAct` 中高风险裸表达已改写为明确非对小P说/不用执行口径；已移除 `ParkingControl` 样本                                                                                                                        |
| Clarify                                | 188  | 已拆为两类样本：`user -> 追问 TTS`（94 条，教模型何时追问）+ 完整 4 轮 `user -> 追问 -> 用户补齐 -> tool call`（94 条，教模型追问后如何响应）；已移除纯位置追问（音区可自动判定位置）与意图明确的方向性追问（如"太晒→打开/关闭遮阳帘"）；配合 final-assistant 标签监督，两类样本均能被正确监督 |
...
`build_train_data.py` 默认递归合并全部 split，当前未加权最终训练集为 9705 条；统计时多轮样本按最后一个有效决策标签计入对应类别。当前未加权输出类型为 Action JSON 5387、MultiAction JSON 数组 18、NoiseDoNotAct 736、TTS 2394、Reject 1170。`NoiseDoNotAct` 以工具 JSON 形式训练，但语义上属于不执行动作边界，训练日志会单独统计 `Noise`，避免和普通 Action 混在一起误判工具化倾向。
