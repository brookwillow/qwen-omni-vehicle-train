...
| Clarify | 202 | 已拆为两类样本：`user -> 追问 TTS`（101 条，教模型何时追问）+ 完整 4 轮 `user -> 追问 -> 用户补齐 -> tool call`（101 条，教模型追问后如何响应）；配合 last-user-anchored 标签监督，两类样本均能被正确监督 |
...
`build_train_data.py` 默认递归合并全部 split，当前最终训练集为 9102 条；统计时多轮样本按最后一个有效决策标签计入对应类别。
