...
| `data/splits/by_tool/*.jsonl` | 按工具拆分的训练数据；每个工具文件内已拆为 `user -> JSON tool call` 决策样本和独立 `JSON tool call -> tool-role JSON result -> TTS text` 回复样本；其中 `NoiseDoNotAct.jsonl` 已扩充到 280 条 |
...
| 类型 | 数量 | 说明 |
|------|------|------|
| By-tool | 6370 | 每个工具文件内混合：`user -> JSON tool call` 决策样本 3909 条，独立 `JSON tool call -> tool-role JSON result -> TTS text` 回复样本 2255 条，以及少量多轮决策上下文 |
...
`build_train_data.py` 默认递归合并全部 split，当前最终训练集为 8818 条；统计时多轮样本按最后一个有效决策标签计入对应类别。
